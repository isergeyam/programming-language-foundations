...
Lemma imply_relf: forall P, P ->> P. 
Proof.
  unfold assert_implies. intros. assumption. 
Qed.

Lemma hoare_asgn_leq: {{X <= Y}} <{Z := X}> {{Z <= Y}}. 
Proof.
...
Proof.
  ...
  Focus 2. eapply hoare_consequence_pre. 
  Focus 2. apply imply_relf. apply hoare_asgn_leq. 
  eapply hoare_seq. apply hoare_asgn. 
  unfold hoare_triple. intros. unfold assn_sub. 
  simpl. rewrite t_update_eq. rewrite t_update_neq. inversion H. 
  subst. rewrite t_update_neq. rewrite t_update_eq. simpl. simpl in *. 
  assumption. intros H1. discriminate H1. intros H1. discriminate H1. 
Qed.
(** [] *)
...
Proof.
  ...
  assert ({{ X = 0 }} X := 3; Y := X {{ Y = 0 }}). 
  { unfold hoare_triple. apply (H X 0). } 
  unfold hoare_triple in *. assert (3 = 0). {
    apply (H0 empty_st (Y !-> 3; X !-> 3)). apply E_Seq with (X !-> 3). 
...
Proof. 
  ...
  - eapply hoare_consequence_pre. 
    ...
    apply eq_true_negb_classical in H2. apply eqb_eq in H2. lia. 
Qed.
(* Do not modify the following line: *)
...
Proof.
  apply E_RepeatFalse with (Y !-> 1 ; X !-> 1). reflexivity. 
  apply E_Seq with (X !-> 1). apply E_Ass. reflexivity. 
  apply E_Ass. reflexivity. apply E_RepeatTrue. reflexivity. 
Qed.
...
Proof.
  unfold hoare_triple. unfold havoc_pre. intros. inversion H; subst. 
  apply (H0 n). 
Qed.
...
Proof.
  ...
  - apply hoare_havoc.
  - unfold "->>". unfold havoc_pre. intros. exists (st X). 
  unfold assn_sub. rewrite t_update_shadow. simpl. rewrite t_update_same. assumption. 
Qed. 
(** [] *)
...
Theorem assert_assume_differ : exists (P:Assertion) b (Q:Assertion),
  ...
  /\ ~ ({{P}} assert b {{Q}}).
Proof.
  exists ((X = 0)%assertion).  exists <{ X = 1 }>. exists ((X = 0)%assertion). split. 
  unfold hoare_triple. intros. simpl in H0. inversion H; subst. simpl in H2. 
  apply eqb_eq in H2. rewrite H0 in H2. discriminate. 
  unfold hoare_triple. intros contra. assert (empty_st =[ assert (X = 1) ]=> RError). 
  { apply E_AssertFalse. reflexivity. } apply contra in H. destruct H as [st' [falso _]]. 
...
Theorem assert_implies_assume : forall P b Q,
  ...
  -> ({{P}} assume b {{Q}}).
Proof.
  unfold hoare_triple. intros. inversion H0; subst. assert (st =[ assert b ]=> RNormal st). 
  { apply E_AssertTrue. assumption.  } apply H in H2. assumption. assumption. 
Qed.
...
Theorem hoare_assert : forall P (b : bexp),
  {{P /\ b}} assert b {{P /\ b}}. 
Proof.
  unfold hoare_triple. intros. destruct H0 as [H0 H1]. inversion H; subst. exists st. 
  split. reflexivity. split; assumption. simpl in H1. rewrite H3 in H1. discriminate. 
Qed.

Theorem hoare_assume : forall P (b : bexp),
  {{P}} assume b {{P /\ b}}. 
Proof.
  unfold hoare_triple. intros. inversion H; subst. exists st. 
...
Example assert_assume_example:
  ...
  {{True}}.
Proof.
  eapply hoare_seq. eapply hoare_seq. Focus 3. apply hoare_assume. 
  (* apply hoare_consequence_post with (Q' := (X = 2)%assertion). apply hoare_assert.  *)
  Focus 2. simpl. unfold hoare_triple. intros. destruct H0 as [_ H0]. rewrite eqb_eq in H0. 
  inversion H; subst. exists ((X !-> aeval st <{ X + 1 }>; st)). split. reflexivity. 
  simpl. rewrite H0. simpl. Admitted. 
