...
Proof.
  ...
  apply hoare_consequence_post 
    with (Q' := ((ap parity X) = (ap parity m) /\ ~ <{2 <= X}>)%assertion). 
  apply hoare_consequence_pre 
    with (P' := ((ap parity X) = (ap parity m))%assertion). 
  - apply hoare_while. apply hoare_consequence_pre 
    with (P' := (ap parity X = ap parity m )%assertion [X |-> X - 2]). 
    apply hoare_asgn. verify_assn. rewrite <- H.  
    apply parity_ge_2. 
    destruct (st X). discriminate. destruct n. discriminate. lia. 
  - verify_assn. 
  - verify_assn. rewrite <- H. symmetry. apply parity_lt_2. destruct (st X). 
  lia. destruct n. lia. discriminate. 
Qed.
...
Proof.
 unfold is_wp. split. 
 eapply hoare_consequence_pre. apply hoare_asgn. verify_assn. 
 intros. verify_assn. unfold hoare_triple in H. eapply (H st) in H0. 
 Focus 2. apply E_Ass. simpl. reflexivity. rewrite t_update_eq in H0. lia. 
Qed.
(** [] *)
...
Theorem hoare_asgn_weakest : forall Q X a,
  is_wp (Q [X |-> a]) <{ X := a }> Q.
Proof.
  unfold is_wp. split. apply hoare_asgn. intros. intros st H1. 
  unfold hoare_triple in H. eapply (H st) in H1. Focus 2. apply E_Ass. 
...
Lemma hoare_havoc_weakest : forall (P Q : Assertion) (X : string),
  ...
  P ->> havoc_pre X Q.
Proof.
  intros. intros st H1. unfold havoc_pre. intros. unfold hoare_triple in H. 
  eapply (H st) in H1. Focus 2. apply E_Havoc. apply H1. 
Qed.
...
Proof.
  verify. 
  - induction (st X). contradiction. simpl. rewrite sub_0_r. 
  assert (fact (S n) = S n * fact n) by reflexivity. rewrite H1 in H. lia. 
  - simpl in H. lia. 
Qed.
...
Proof.
  intros. destruct n. assert (~ (0 > 0)) by lia. apply H0 in H. destruct H. 
  destruct n. reflexivity. 
  assert (fib(S (S n)) = fib (S n) + fib n) by reflexivity. 
  assert (fib(S (S (S n))) = fib (S (S n)) + fib (S n)) by reflexivity. 
  assert (pred (S (S n)) = S n) by reflexivity. 
  assert (1 + (S (S n)) = S((S(S(n))))) by reflexivity. 
  rewrite H0. rewrite H3. rewrite H1. rewrite H2. rewrite H0. lia. 
Qed.
(** [] *)
...
Definition dfib (n : nat) : decorated := <{
    ...
    {{1 > 0 /\ 1 = fib 1 /\ 1 = fib 0}}
    X := 1
    {{X > 0 /\ 1 = ap fib X /\ 1 = ap fib (X - 1)}};
    Y := 1
    {{X > 0 /\ 1 = ap fib X /\ Y = ap fib (X - 1)}};
    Z := 1
    {{X > 0 /\ Z = ap fib X /\ Y = ap fib (X - 1)}};
    while ~(X = 1 + n) do
      {{X > 0 /\ Z = ap fib X /\ Y = ap fib (X - 1) /\ ~(X = 1 + n)}} ->>
      {{1 + X - 1 > 0 /\ Z + Y = ap fib (1 + X) /\ Z = ap fib (1 + X - 1) }}
      T := Z
      {{ 1 + X - 1 > 0 /\ Z + Y = ap fib (1 + X) /\ T = ap fib (1 + X - 1) }};
      Z := Z + Y
      {{ 1 + X - 1 > 0 /\ Z = ap fib (1 + X) /\ T = ap fib (1 + X - 1) }};
      Y := T
      {{ 1 + X - 1 > 0 /\ Z = ap fib (1 + X) /\ Y = ap fib (1 + X - 1) }};
      X := 1 + X
      {{ X - 1 > 0 /\ Z = ap fib X /\ Y = ap fib (X - 1) }} ->>
      {{ X > 0 /\ Z = ap fib X /\ Y = ap fib (X - 1) }}
    end
      {{ Z = ap fib X /\ Y = ap fib (X - 1) /\ X = 1 + n}} ->>
      {{ Y = fib n }}
}>.
...
| DCWhile (b : bexp) (Pinv : Assertion) (d : dcom)
  (* while b do d end {{Pinv}}*)
.
...
Notation "l ':=' a"
      := (DCAsgn l a)
      (in custom com at level 0, l constr at level 0,
          a custom com at level 85) : dcom_scope. 
Notation "'while' b 'do' d 'end' {{ Pinv }}"
      := (DCWhile b Pinv d)
           (in custom com at level 89, b custom com at level 99,
           Pinv constr) : dcom_scope. 
Notation "'if' b 'then' d 'else' d' 'end'"
      := (DCIf b d d')
           (in custom com at level 89, 
           b custom com at level 99, 
           d custom com at level 99, 
           d' custom com at level 99) : dcom_scope. 
Notation " d ; d' "
      := (DCSeq d d')
      (in custom com at level 90, right associativity) : dcom_scope. 
Notation "{{ P }} d {{ Q }}"
...
Fixpoint pre  (d : dcom) (Q : Assertion) : Assertion :=
  match d with
  | DCSkip                  => Q
  | DCSeq d1 d2             => pre d1 (pre d2 Q)
  | DCAsgn X a              => Q [X |-> a]
  | DCIf  b d1 d2           => (pre d1 Q /\ b) \/ (pre d2 Q /\ ~ b)
...
Proof.
  ...
  - destruct H as [H1 [H2 H3]]. eapply hoare_consequence; eauto. 
