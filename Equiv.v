...
Example optimize_0plus_ex1 : optimize_0plus_aexp <{ (0 + X) * (1 + (0 + Y)) }> =
  <{ X * (1 + Y) }>. 
Proof. reflexivity. Qed. 
...
Ltac rewrite_IH H1 H2 := rewrite H1; rewrite H2; reflexivity. 

Theorem optimize_0plus_aexp_sound: atrans_sound optimize_0plus_aexp. 
Proof.
  unfold atrans_sound. intros a. unfold aequiv. intros st. 
  induction a; simpl; try reflexivity; try (rewrite_IH IHa1 IHa2). 
  - destruct a1; try (rewrite_IH IHa1 IHa2). 
    destruct n. reflexivity. rewrite_IH IHa1 IHa2. 
Qed.
...
Definition optimize_0plus_fold_com c := fold_constants_com (optimize_0plus_com c). 

Theorem optimize_0plus_fold_com_sound: ctrans_sound optimize_0plus_fold_com. 
Proof.
  unfold ctrans_sound. unfold optimize_0plus_fold_com. intros c. 
  apply trans_cequiv with (optimize_0plus_com c). apply optimize_0plus_com_sound. 
...
Theorem subst_equiv: subst_equiv_property'. 
Proof.
  unfold subst_equiv_property'. intros. split. 
  - intros H1. remember (x1 !-> (aeval st a1) ; st) as st''. 
    (* apply E_Seq with st''. rewrite Heqst''. 
    apply E_Ass. reflexivity. eapply aeval_weakening in H.  *)
    assert (H2: st =[ x1 := a1; x2 := a2 ]=> (x2 !-> (aeval st'' a2); x1 !-> (aeval st a1); st)).
    { apply E_Seq with st''. rewrite Heqst''. apply E_Ass. reflexivity. 
      rewrite Heqst''. apply E_Ass. reflexivity. } 
    assert (H3: st' = (x2 !-> (aeval st'' a2); x1 !-> (aeval st a1); st)). {
      eapply ceval_deterministic. apply H1. apply H2. } rewrite H3. 
      rewrite Heqst''. rewrite aeval_weakening. 
Admitted.
...
Proof.
  ...
  remember (t_empty 0) as st.
  assert (H: st =[ skip ]=> st) by (apply E_Skip). 
  rewrite <- contra in H. 
...
Proof. apply E_Seq with empty_st. apply E_Skip. apply E_Havoc. Qed. 
...
Proof. 
  unfold cequiv. unfold pXY. unfold pYX.  intros st st'. split. 
  - intros H. inversion H. subst. inversion H2. subst. inversion H5. subst. 
    apply E_Seq with (Y !-> n0; st). apply E_Havoc. 
    assert (H3: (Y !-> n0; X !-> n; st) = (X !-> n; Y !-> n0; st)). {
    apply t_update_permute. intros contra. discriminate. }
    rewrite H3. apply E_Havoc. 
  - intros H. inversion H. subst. inversion H2. subst. inversion H5. subst. 
    apply E_Seq with (X !-> n0; st). apply E_Havoc. 
    assert (H3: (X !-> n0; Y !-> n; st) = (Y !-> n; X !-> n0; st)). {
    apply t_update_permute. intros contra. discriminate. }
    rewrite H3. apply E_Havoc. 
Qed.
(** [] *)
...
Theorem ptwice_cequiv_pcopy :
  ~cequiv ptwice pcopy.
Proof.
  unfold cequiv. unfold ptwice. unfold pcopy. intros contra. 
  remember empty_st as st. 
  remember (Y !-> 1; X !-> 0) as st'. 
  assert (H: st =[ havoc X; havoc Y ]=> st'). { 
    rewrite Heqst. rewrite Heqst'. 
    apply E_Seq with (X !-> 0); apply E_Havoc. }
  rewrite contra in H. subst. inversion H. subst. inversion H2. subst. 
  inversion H5. subst. simpl in H6. 
  assert ((Y !-> 1; X !-> 0) X = 0) by reflexivity. 
  assert ((Y !-> 1; X !-> 0) Y = 1) by reflexivity.  
  assert ((Y !-> (X !-> n) X; X!-> n) X = n) by reflexivity. 
  assert ((Y !-> (X !-> n) X; X!-> n) Y = n) by reflexivity. 
  rewrite <- H6 in H0. rewrite H3 in H0. 
  rewrite <- H6 in H1. rewrite H4 in H1. 
...
Lemma p1_may_diverge : forall st st', st X <> 0 ->
  ~ st =[ p1 ]=> st'.
Proof. 
  unfold p1. intros. intros contra. 
  remember <{ while ~ (X = 0) do
       havoc Y;
       X := X + 1
     end }> as loopdef. 
  induction contra; try (subst; discriminate). 
  - injection Heqloopdef as H1 H2. rewrite H1 in H0. simpl in H0. 
  apply negb_false_iff in H0. rewrite eqb_eq in H0. apply H in H0. contradiction. 
  - injection Heqloopdef as H1 H2. rewrite H2 in contra1. 
    inversion contra1. subst. inversion H8. subst. simpl in IHcontra2. 
    assert (H1: (X !-> st'0 X + 1; st'0) X = st'0 X + 1) by reflexivity. 
    assert (H2: st'0 X + 1 <> 0) by lia. rewrite <- H1 in H2. clear H1. 
    apply IHcontra2. apply H2. reflexivity. 
Qed.

Lemma p2_may_diverge : forall st st', st X <> 0 ->
  ~ st =[ p2 ]=> st'.
Proof.
  unfold p2. intros. intros contra. 
  remember 
  <{ while ~ (X = 0) do
       skip
     end }> as loopdef. 
  ...
  - injection Heqloopdef as H1 H2. rewrite H1 in H0. simpl in H0. 
    apply negb_false_iff in H0. rewrite eqb_eq in H0. apply H in H0. contradiction. 
  - injection Heqloopdef as H1 H2. subst. inversion contra1. subst. apply IHcontra2. 
...
Proof. 
  unfold p1. unfold p2. intros st st'. split. 
  - intros H. inversion H. subst. apply E_WhileFalse. assumption. 
    subst. inversion H3. subst. inversion H8. subst. 
    simpl in H6. 
    assert (H1: (X !-> st'1 X + 1; st'1) X = st'1 X + 1) by reflexivity. 
    assert (H9: st'1 X + 1 <> 0) by lia.  rewrite <- H1 in H9. 
    apply (p1_may_diverge (X !-> st'1 X + 1; st'1) st' H9) in H6. contradiction. 
  - intros H. inversion H. subst. apply E_WhileFalse. assumption. 
    subst. inversion H3. subst. simpl in H2. apply negb_true_iff in H2. 
    assert (H4: st'0 X <> 0). { intros contra. 
    rewrite <- eqb_eq in contra. rewrite contra in H2. discriminate. } 
    apply (p2_may_diverge st'0 st' H4) in H. contradiction. 
Qed.
(** [] *)
...
Proof.
  unfold p3. unfold p4. intros contra. 
  remember (X !-> 1) as st. 
  remember (Z !-> 2; X !-> 0; Z !-> 1; X !-> 1) as st'. 
  assert (H: st =[ p3 ]=> st'). {
    unfold p3. rewrite Heqst. rewrite Heqst'. 
    apply E_Seq with (Z !-> 1; X !-> 1). apply E_Ass. reflexivity. 
    apply E_WhileTrue with (Z !-> 2; X !-> 0; Z !-> 1; X !-> 1). reflexivity. 
    apply E_Seq with (X !-> 0; Z !-> 1; X !-> 1); apply E_Havoc. apply E_WhileFalse. 
    reflexivity.  
  }
  unfold cequiv in contra. unfold p3 in H. rewrite contra in H. 
  inversion H. subst. inversion H2. subst. simpl in H5. inversion H5. subst. simpl in H6. 
  assert (H1: (Z !-> 1; X !-> 0; X !-> 1) Z = 1) by reflexivity. 
  rewrite H6 in H1. 
  assert (H3: (Z !-> 2; X !-> 0; Z !-> 1; X !-> 1) Z = 2) by reflexivity. 
...
Lemma p5_halt_criterion: forall st st', st =[p5]=> st' <-> st' = (X !-> 1; st). 
Proof.
  unfold p5. intros. split. 
  + intros. 
    remember <{ while ~(X = 1) do
          havoc X
        end }> as body. 
    induction H; try (subst; discriminate). 
    - injection Heqbody as H1 H2. subst. simpl in H. 
    apply negb_false_iff in H. 
    apply eqb_eq in H. 
    rewrite <- H. symmetry. apply t_update_same. 
    - injection Heqbody as H2 H3. subst. 
    assert (Heq: <{ while ~ X = 1 do havoc X end }> = <{ while ~ X = 1 do havoc X end }>) by reflexivity. 
    apply IHceval2 in Heq. inversion H0. subst. apply t_update_shadow. 
  + intros. destruct (eqbP (st X) 1). 
    rewrite <- e in H. assert (H1: (X !-> st X; st) = st) by (apply t_update_same). 
    rewrite H1 in H. rewrite H. apply E_WhileFalse. apply negb_false_iff. 
    apply eqb_eq. simpl. assumption. 
    apply E_WhileTrue with st'. simpl. apply negb_true_iff. apply eqb_neq. assumption. 
    rewrite H. eapply E_Havoc. apply E_WhileFalse. 
    apply negb_false_iff. apply eqb_eq. rewrite H. simpl. reflexivity. 
Qed.

Lemma p6_halt_criterion: forall st st', st =[p6]=> st' <-> st' = (X !-> 1; st). 
Proof.
  intros. unfold p6. split. 
  - intros. inversion H. subst. reflexivity. 
  - intros. rewrite H. apply E_Ass. reflexivity. 
Qed.

Theorem p5_p6_equiv : cequiv p5 p6.
Proof. 
  unfold cequiv. intros. split. 
  - intros H. apply p5_halt_criterion in H. apply p6_halt_criterion. assumption. 
  - intros H. apply p6_halt_criterion in H. apply p5_halt_criterion. assumption. 
Qed.
(** [] *)
...
Proof.
  intros. split. 
  - intros. inversion H2. subst. inversion H5. subst. inversion H8. subst. 
    apply E_Seq with (l2 !-> aeval st a2; st). apply E_Ass. reflexivity. 
    assert (Hass: (l2 !-> aeval st a2; st) =[ l1 := a1 ]=> 
    (l1 !-> aeval (l2 !-> aeval st a2; st) a1; l2 !-> aeval st a2; st)). 
    { apply E_Ass. reflexivity. } 
    assert (Heq: (l2 !-> aeval (l1 !-> aeval st a1; st) a2; l1 !-> aeval st a1; st) = 
    (l1 !-> aeval (l2 !-> aeval st a2; st) a1; l2 !-> aeval st a2; st)). 
...
Proof. 
  intros. unfold capprox. split. 
  - intros contra. 
  assert (H1: empty_st =[ c3 ]=> (X !-> 1; empty_st)). 
  { apply E_Ass. reflexivity. } 
  assert (H2: empty_st =[ c4 ]=> (Y !-> 1; empty_st)). 
  { apply E_Ass. reflexivity. } 
  apply contra in H1. apply (ceval_deterministic c4 empty_st (X !-> 1) (Y !-> 1)) in H2. 
  assert (H3: aeval (X !-> 1) X = 1) by reflexivity. rewrite H2 in H3. simpl in H3. 
  discriminate. assumption. 
  - intros contra. 
  assert (H2: empty_st =[ c3 ]=> (X !-> 1; empty_st)). 
  { apply E_Ass. reflexivity. } 
  assert (H1: empty_st =[ c4 ]=> (Y !-> 1; empty_st)). 
  { apply E_Ass. reflexivity. } 
  apply contra in H1. apply (ceval_deterministic c3 empty_st (X !-> 1) (Y !-> 1)) in H2. 
  assert (H3: aeval (X !-> 1) X = 1) by reflexivity. rewrite H2 in H3. simpl in H3. 
  discriminate. assumption. 
Qed.
...
Proof.
  unfold capprox. intros. apply loop_never_stops in H. contradiction. 
Qed.
